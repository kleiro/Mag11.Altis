...
buttonSetAction[2400,"
	if((missionNamespace getVariable ['convoyArrived', false]) isEqualTo false) then {
		switch (lbCurSel 2101) do {
			case 0 : {
				missionNamespace setVariable ['ConvoyAA', 0, true];
			};
			case 1 : {
				missionNamespace setVariable ['ConvoyAA', 1, true];
			};
		};
		missionNamespace setVariable ['convoyHuntEnabled', true, true];
		closeDialog 1;
	} else {
...
missionNamespace setVariable ['convoyArrived', 'canceled', true];
closeDialog 1;"];